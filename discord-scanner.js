...
function normalizeColor(value, fallback) {
    ...
    // If it's already a number, convert to hex
    ...
    if (typeof raw === "string") {
        ...
        // If it's a 6-hex string like "4854e9"
        if (/^[0-9a-fA-F]{6}$/.test(s)) return "#" + s.toLowerCase();
        if (/^[0-9a-fA-F]{3}$/.test(s)) return "#" + s.toLowerCase();
        // Try to parse decimal string
...
function getRoleStyleProps(role) {
    ...
    const primary =
        normalizeColor(role.primaryColor, roleColor) ||
        normalizeColor(roleColor) ||
        "#99aab5";
    ...
    if (!role.roleStyle) {
        if (role.primaryColor && role.secondaryColor)
            effectiveStyle = "gradient";
        else if (role.holographic || role.isHolographic)
            effectiveStyle = "holographic";
...
function updateServerInfo() {
    if (!discordData || !discordData.guild) return;

    // Défensive: normaliser le nom du serveur
    const guildNameRaw = discordData.guild.name;
    const guildName =
        typeof guildNameRaw === "string" && guildNameRaw.trim() !== ""
            ? guildNameRaw
            : "Server";
    const guildNameUpper = guildName.toUpperCase();

    // Mettre à jour le nom du serveur
    const serverName = document.querySelector(".server-header h3");
    if (serverName) {
        serverName.textContent = guildName;
    }
    ...
    if (categoryTitle) {
        categoryTitle.textContent = guildNameUpper;
    }
    ...
    if (serverIcon && discordData.guild.icon) {
        serverIcon.innerHTML = `<img src="${discordData.guild.icon}" alt="${guildName}" style="width: 100%; height: 100%; border-radius: inherit;">`;
    } else if (serverIcon) {
        ...
        const initials = guildName
            ...
            .join("")
            .substring(0, 2);
        serverIcon.textContent = initials || "S";
    }
    ...
    if (headerIcon && discordData.guild && discordData.guild.icon) {
        if (headerIcon.tagName === "IMG") {
            headerIcon.src = discordData.guild.icon;
        } else {
            headerIcon.innerHTML = `<img src="${discordData.guild.icon}" alt="${guildName}" style="width: 20px; height: 20px; border-radius: 50%; margin-right: 8px;">`;
        }
    }
}
...
function viewChannelMessages(channelId, channelName) {
    ...
    messages.forEach((message) => {
        ...
        // Apply role-derived style to the username to ensure gradients/holographic styles override inline colors
        try {
            const msgNameEl = messageDiv.querySelector(".message-username");
            if (msgNameEl) {
                const props = getRoleStyleProps(highestRole);
                if (props.effectiveStyle === "gradient") {
                    msgNameEl.style.setProperty(
                        "background",
                        `linear-gradient(90deg, ${props.primary} 0%, ${props.secondary} 100%)`,
                        "important",
                    );
                    msgNameEl.style.setProperty(
                        "background-clip",
                    ...
                    msgNameEl.style.setProperty(
                        "background",
                        "linear-gradient(90deg, #ff73fa 0%, #b968c7 20%, #00d4aa 40%, #00ffff 60%, #b968c7 80%, #ff73fa 100%)",
                        "important",
                    );
                    msgNameEl.style.setProperty(
                        "background-clip",
            ...
            // Non-fatal: styling failure shouldn't break message rendering
            console.error("Erreur application style nom message:", e);
        }
    });
}
...
function createMessageElement(message) {
    ...
    if (isBot && embeds && embeds.length > 0) {
        ...
        // Do not inline-style here; we'll apply the resolved role style programmatically below using getRoleStyleProps()
    ...
    try {
        ...
        if (highestRole) {
            if (isRoleObjectArray) highestRoleObj = highestRole;
                ...
                if (Array.isArray(discordData.roles)) {
                    highestRoleObj = discordData.roles.find(
                        (r) => String(r.id) === String(highestRole),
                    );
                } else {
...
let roleMembersCurrentPage = 0;
let roleMembersPerPage = 100;
let roleMembersTotalLoaded = 0;
let roleMembersContainer = null;
...
function loadRoleMembers(roleId) {
    ...
    if (roleMembersArray.length > 100) {
        ...
        roleMembersLoadInterval = setInterval(() => {
            ...
            currentIndex += batchSize;
            console.log(
                `📥 ${currentIndex}/${roleMembersArray.length} membres chargés`,
            );
        }, 1000); // 1 seconde par batch de 100
    }

    // Gérer la recherche
    const searchInput = membersTab.querySelector(".role-members-search-input");
    if (searchInput) {
        searchInput.addEventListener("input", (e) => {
            const searchTerm = e.target.value.toLowerCase();
            const memberItems =
                roleMembersList.querySelectorAll(".role-member-item");
...
// Fonction pour créer l'élément d'un membre dans la liste des rôles
function createRoleMemberElement(member, roleId) {
    ...
    // Récupérer la couleur et le style du rôle associé
    const roleData = discordData.roles.find(
        (r) => String(r.id) === String(roleId),
    );
    const roleColor =
        roleData?.color && roleData.color !== "#000000"
            ? roleData.color
            : "#99aab5";
    const roleStyle = roleData?.roleStyle || "solid";
    const primaryColor = roleData?.primaryColor;
    const secondaryColor = roleData?.secondaryColor;

    memberDiv.innerHTML = `
        <div class="member-avatar">
            <img src="${member.avatar}" alt="${member.displayName}" loading="lazy" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
        </div>
        <div class="member-details">
            <div class="role-member-name">${member.displayName}</div>
            <div class="role-member-username">@${member.username}</div>
        </div>
        <button class="remove-member-btn" onclick="removeMemberFromRole('${roleId}', '${member.userId}')">Remove</button>
    `;

    // Appliquer le style du rôle de manière programmatique pour garantir la cohérence
    try {
...
function removeMemberFromRole(roleId, userId) {
    console.log(
        `ℹ️ Fonction removeMemberFromRole appelée pour l'utilisateur ${userId} du rôle ${roleId}`,
    );
    ...
    if (membersTabButton) {
        ...
        window.showRoleTab = function (tabName) {
            if (tabName !== "members" && roleMembersLoadInterval) {
                clearInterval(roleMembersLoadInterval);
                roleMembersLoadInterval = null;
                console.log("✅ Intervalle de chargement des membres arrêté");
            }
...
async function populateRoleData(role) {
    ...
    // Gérer le style de rôle (solid, gradient, holographic) - DÉCLARER AVANT UTILISATION
    const roleStyle = role.roleStyle || "solid";
    const primaryColor = role.primaryColor;
    const secondaryColor = role.secondaryColor;
    const tertiaryColor = role.tertiaryColor;
    // Couleur principale (fallback) utilisée par plusieurs blocs
    const roleColor =
        role.color && role.color !== "#000000" ? role.color : "#99aab5";

    // Gérer les boutons de style de rôle
    const roleStyleButtons = document.querySelectorAll(".role-style-icon");

    if (roleStyleButtons.length > 0) {
        // Retirer toutes les sélections
        roleStyleButtons.forEach((btn) => btn.classList.remove("selected"));

        // Sélectionner le bon bouton selon le style
        if (roleStyle === "solid") {
            roleStyleButtons[0]?.classList.add("selected");
        } else if (roleStyle === "gradient") {
            roleStyleButtons[1]?.classList.add("selected");
        } else if (roleStyle === "holographic") {
            roleStyleButtons[2]?.classList.add("selected");
        }

        // Afficher/masquer les modes de couleur appropriés
        ...
        if (solidMode)
            solidMode.classList.toggle("hidden", roleStyle !== "solid");
        if (gradientMode)
            gradientMode.classList.toggle("hidden", roleStyle !== "gradient");
        if (holographicMode)
            holographicMode.classList.toggle(
                "hidden",
    ...
    if (roleStyle === "gradient" && primaryColor && secondaryColor) {
        const gradientMode = document.getElementById("gradientColorMode");
        if (gradientMode) {
            gradientMode.classList.remove("hidden");

            // Mettre à jour la prévisualisation du gradient
            const gradientPreview = document.getElementById("gradientPreview");
            if (gradientPreview) {
                const color1 =
                    normalizeColor(primaryColor, roleColor) ||
                    normalizeColor(roleColor) ||
                    "#99aab5";
                const color2 = normalizeColor(secondaryColor, color1) || color1;
                const color3 = normalizeColor(tertiaryColor, color1) || color1;
...
async function loadMembers() {
    ...
    try {
        ...
        // Normaliser les rôles en une map cléée par id (certaines sources renvoient un tableau)
        const guildRoles = {};
        ...
        if (!membersContainer) return; // Vérification supplémentaire
        ...
        sortedRoles.forEach((roleData) => {
            ...
            // Appliquer le style du rôle au nom (solid / gradient / holographic)
            try {
                const roleNameSpan = roleHeader.querySelector(".role-name");
                if (roleNameSpan) {
                    const props = getRoleStyleProps(roleData);
                    if (props.effectiveStyle === "gradient") {
                        roleNameSpan.style.setProperty(
                            "background",
                            `linear-gradient(90deg, ${props.primary} 0%, ${props.secondary} 100%)`,
                            "important",
                        );
                ...
                console.warn("Erreur application style rôle:", e);
            }
            ...
            // Afficher les membres immédiatement
...
function createMemberElement(member, roleData) {
    ...
    // Résolution robuste de l'avatar (plusieurs formats possibles selon la source)
    ...
    const avatarUrl =
        avatarCandidates.find((v) => v && typeof v === "string") ||
    ...
    memberDiv.innerHTML = `
        <div class="member-avatar-wrapper">
            ...
            <img src="assets/OfflineStatus.png" alt="offline" class="member-status-icon">
        </div>
    ...
    if (memberNameElement) {
        try {
            const props = getRoleStyleProps(roleData);
            if (props.effectiveStyle === "gradient") {
                memberNameElement.style.setProperty(
                    "background",
                    `linear-gradient(90deg, ${props.primary} 0%, ${props.secondary} 100%)`,
                    "important",
                );
